...
int* findOrder(int numCourses, int** prerequisites, int prerequisitesSize, int* prerequisitesColSize, int* returnSize)
{
    ...
    stack s;
    s.top = -1;
    s.data = (int *)malloc(sizeof(int) * numCourses);

    for(int i = 0; i < prerequisitesSize; i++)
        degree[prerequisites[i][0]]++; // 计算所有顶点的入度

    for(int i = 0; i < numCourses; i++)
        if(degree[i] == 0) s.data[++s.top] = i; // 将入度为0顶点入栈

    int cnt = 0;

    while(s.top > -1){
        int e = s.data[s.top--]; // 弹出栈顶顶点v
        res[cnt++] = e;
        for(int i = 0; i < prerequisitesSize; i++){
            if(prerequisites[i][1] == e) { 
                degree[prerequisites[i][0]]--; // 与v相连的w入度减一
                if(degree[prerequisites[i][0]] == 0) 
                    s.data[++s.top] = prerequisites[i][0]; // 将w进栈
            }
        }
    }
